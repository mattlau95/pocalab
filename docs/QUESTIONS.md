...
8. **Crop output: 696 or 697 px wide?** 59 mm at 300 DPI is 696.85 px. The code has always rounded down to 696 × 1051, while the README and devlog said 697 × 1051 (I've corrected the docs to match the code). Rounding to nearest would give 697, which is closer to true 59 mm (59.01 vs 58.93 mm) and to the card's aspect ratio; the difference is under 0.1 mm and invisible in print. Switching changes every new crop by 1 px, and auto-fill would need to accept both widths so older exports still match. Keep 696, or switch?
